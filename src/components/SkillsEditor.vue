  ...
  <section class="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-900 sm:p-4">
    ...
    <div class="space-y-3">
      <div v-for="(group, index) in groups" :key="group.id" class="grid gap-2">
        ...
        <button type="button" class="inline-flex w-full items-center justify-center gap-2 rounded-md bg-rose-600 px-3 py-2 text-sm text-white sm:w-fit" @click="removeGroup(index)">
          <Trash2 :size="15" />
          Remove
        </button>
      </div>
